...
Server có thể push messages đến clients mà không cần request. Các broadcast messages có `rid.id < 0` để phân biệt với response thông thường.
...
Server sử dụng batch broadcast để tối ưu performance. Thay vì gửi từng message riêng lẻ, server gom nhiều messages lại và gửi mỗi 1 giây.
